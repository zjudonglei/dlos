...
PUBLIC int do_disklog() {
	...
	int str_len = fs_msg.CNT;
	int src = fs_msg.source;
	assert(str_len < STR_DEFAULT_LEN);
	phys_copy((void*)va2la(TASK_FS, buf), (void*)va2la(src, fs_msg.BUF), str_len);
	buf[str_len] = 0;
...
PUBLIC int disklog(char* logstr) {
	...
	int nr_log_blk0_nr = sb->nr_sects - NR_SECTS_FOR_LOG; // 在末尾预留2048个扇区用于存储日志
	static int pos = 0; // 只会初始化一次，后面每次都是上次更改后的值
	if (!pos) { // 第一次执行，sector-map中记录这些扇区被使用
		...
		for (i = nr_log_blk0_nr; i < nr_log_blk0_nr + NR_SECTS_FOR_LOG; i += chunk) {
			...
			rw_sector(DEV_WRITE, device, i * SECTOR_SIZE, chunk * SECTOR_SIZE, TASK_FS, fsbuf);
			sects_left -= chunk;
		}
		if (sects_left != 0)
	...
	WR_SECT(device, (u32)(nr_log_blk0_nr + NR_SECTS_FOR_LOG - 1));
...
PUBLIC void dump_fd_graph(const char* fmt, ...) {
	...
	for (i = 0; i < NR_TASKS + NR_PROCS; i++, p_proc++) {
		...
		"|<f1> p_flags:%d(%s)",
			p_proc->name, i,
			p_proc->p_flags, proc_flags[p_proc->p_flags]);
		int fnr = 3;
		for (k = 0; k < NR_FILES; k++) {
			if (p_proc->filp[k] == 0)
				continue;
			...
			SYSLOG("\t|<f%d> filp[%d]: %d", fnr, k, fdesc_tbl_idx);
			pfm[pfm_idx].pid = i;
	...
	for (i = 0; i < NR_FILE_DESC; i++) {
		...
		"|<f1> fd_mode:%d"
			"|<f2> fd_pos:%d"
			"|<f4> fd_inode:%d",
			i, f_desc_table[i].fd_mode, f_desc_table[i].fd_pos, inode_tbl_idx);
	...
	for (i = 0; i < sb->nr_smap_sects; i++) {
		RD_SECT(root_inode->i_dev, (u32)(smap_blk0_nr + i));
			...
			for (k = 0; k < 8; k++) {
				if (!smap_flag) {
					...
					if ((_buf[j] >> k) & 1) {
						...
						SYSLOG("\t\t\"sector %xh\" [\n", bit_start);
						SYSLOG("\t\t\tlabel = \"<f0>sect %xh-%xh", bit_start, bit_end);
						SYSLOG("\t\"\n");
						SYSLOG("\t\t\tshape = \"record\"\n");
						SYSLOG("\t\t];\n");
					}
	...
	for (i = 0; i < sb->nr_imap_sects; i++) {
		RD_SECT(root_inode->i_dev, (u32)(imap_blk0_nr + i));
		memcpy(_buf, fsbuf, SECTOR_SIZE);
		for (j = 0; j < SECTOR_SIZE; j++) {
			for (k = 0; k < 8; k++) {
				if ((_buf[j] >> k) & 1) {
	...
	for (i = 0; i < SECTOR_SIZE / sizeof(struct inode); i++, p+=INODE_SIZE) {
		...
		if (pinode->i_nr_sects) {
			...
			SYSLOG("\t\t\tlabel = \"<f0> %d"
				"|<f2> i_size:0x%x"
				"|<f3> sect: %x-%xh", i + 1, pinode->i_size, start_sect, end_sect);
			SYSLOG("\t\"\n");
			SYSLOG("\t\t\tshape = \"record\"\n");
			SYSLOG("\t\t];\n");
		}
			...
			SYSLOG("\t\t\tlabel = \"<f0> %d"
				"|<f2> i_size:0x%x"
				"|<f3> dev nr: (%xh, %xh)", i + 1, pinode->i_size, start_sect, end_sect);
			SYSLOG("\t\"\n");
			SYSLOG("\t\t\tshape = \"record\"\n");
			SYSLOG("\t\t];\n");
		}
	...
	for (i = 0; i < nr_dir_blks; i++) {
		RD_SECT(root_inode->i_dev, (u32)(dir_blk0_nr + i));
		memcpy(_buf, fsbuf, SECTOR_SIZE);
