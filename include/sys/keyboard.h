...
#define KB_IN_BYTES 32 // 缓冲区大小
#define MAP_COLS 3 // keymap 3列
#define NR_SCAN_CODES 0x80 // 一行的字符数

#define FLAG_BREAK 0x0080 // 弹起=按下+这个数
#define FLAG_EXT 0x0100
// 特殊标志，00010000中的1和FLAG_EXT不会重叠
...
#define MASK_RAW 0x01FF // and一个key之后就得到下面的标记
...
#define PAD_UP PAD_8 // 右侧数字键盘区域，8和向上箭头是一个键
...
typedef struct s_kb {
	char* p_head; // 指向下一个空闲位置
	char* p_tail; // 指向下一个待处理的字节
	int count;
